...
import com.blackduck.zap.srm.SrmExtension;
...
public class SSLConnectionSocketFactoryFactory {
	 ...
	 * with the returned socket factory, if the server's certificate appears to be invalid, the user
	 * will be prompted to either reject temporarily accept, or permanently accept the certificate.
	 * Permanently accepted certificates will be stored in a <code>.truststore</code> file so the
	 * user won't need to prompted again for the same host.
	 *
	 * @param host      The host (URL component) that the socket factory will be used to connect to
	 * @param extension
	...
	/**
	 * Determines the location for the <code>truststore</code> file for the given host. Each {@link
	 * SSLConnectionSocketFactory} returned by {@link #initializeFactory(String, SrmExtension,
	 * String, boolean)} needs to have a file to store user-accepted invalid certificates; these
	 * files will be stored in the user's OS-appropriate "appdata" directory.
	 *
	 * @param host A URL hostname, e.g. "www.google.com"
	 * @return The file where the trust store for the given host should be stored
	 */
